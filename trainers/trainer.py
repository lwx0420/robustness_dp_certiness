...
from .utils_awp import AdvWeightPerturb, AdvWeightPerturbOneRound
...
class BaseTrainer:
    ...
    def _train_epoch(self, exp_adv_loss, param_eigenvals, param_for_step=None):
        ...
        with BatchMemoryManager(data_loader=self.train_loader, max_physical_batch_size=self.physical_batch_size, optimizer=self.optimizer) as memory_safe_data_loader:
            ...
            for _batch_idx, (data, target, image_idx) in enumerate(tqdm(memory_safe_data_loader)):
                ...
                # add perturbation
                # awp = self.awp_adversary.calc_awp(inputs_adv=data,
                #                              targets=target)
                # self.awp_adversary.perturb(awp)

                awp = self.awp_adversary.calc_awp()
                self.awp_adversary.perturb(awp)
    ...
    def train(self, write_checkpoint=True):
        ...
        validation_metrics = defaultdict(list)
        self._validate(validation_metrics)
        self.proxy_opt = torch.optim.SGD(self.proxy_model.parameters(), lr=0.01)
        # self.awp_adversary = AdvWeightPerturb(model=self.model, proxy=self.proxy_model, proxy_optim=self.proxy_opt, gamma=0.01)
        learning_rate = self.optimizer.param_groups[-1]['lr']
        self.awp_adversary = AdvWeightPerturbOneRound(model=self.model, gamma=2.0, lr=learning_rate)
        while self.epoch < self.max_epochs:
            self.metric_collection.mark_epoch_start()
            epoch_start_time = time.time()
            self.model.train()
